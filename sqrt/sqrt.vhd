...
architecture rtl of sqrt is
  ...
  signal root       : unsigned(G_DWIDTH-1 downto 0);
  signal root_sqr   : unsigned(G_DWIDTH*2-1 downto 0);

  signal input_reg  : unsigned(G_DWIDTH*2-1 downto 0);
        ...
        case state is
          ...
          when SM_CALC1 =>
            if root_sqr > input_reg then
              root(G_DWIDTH-to_integer(counter)-1) <= '0';
            end if;

            if G_DWIDTH-to_integer(counter)-2 >= 0 then
              root(G_DWIDTH-to_integer(counter)-2) <= '1';
            end if;

            if counter = G_DWIDTH-1 then
